...
# ✅ Supabase Session Pooler URL (IPv4 compatibile per Render)
DATABASE_URL = "postgresql://postgres.ewciddbqysievcwrffhe:ancoracas25@aws-0-eu-west-3.pooler.supabase.com:5432/postgres"
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                print("❌ Connessione a Supabase fallita dopo tutti i tentativi.")
                return False
    return False
...
def archive():
    try:
        ...
        # Pre-validate all guest data to catch any issues before rendering
        for guest in guests:
            if guest.birth_date and hasattr(guest.birth_date, 'year'):
                # Ensure birth_date is properly formatted
                if isinstance(guest.birth_date, datetime):
                    guest.birth_date = guest.birth_date.date()
            
        return render_template('archive.html', guests=guests, now=now)
    except Exception as e:
        import traceback
        app.logger.error(f"Errore nella route archive: {str(e)}")
        traceback.print_exc()  # Stampa l'errore nei log
        flash('Errore durante il caricamento della lista ospiti', 'danger')
        return redirect(url_for('dashboard'))
